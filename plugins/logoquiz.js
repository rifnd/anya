...
Bonus: ${poin} XP
    `.trim()
  conn.logoquiz[id] = [
    await conn.sendButtonImg(m.chat, await (await fetch(json.img)).buffer(), caption, wm, 'Bantuan', '.hints', m)
    ,
    json, poin,
    setTimeout(async () => {
      if (conn.logoquiz[id]) await conn.sendButton(m.chat, `Waktu habis!\nJawabannya adalah *${json.jawaban}*`, wm, 'Logo Quiz', '.logoquiz', conn.logoquiz[id][0])
